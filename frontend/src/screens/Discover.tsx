...
import { DISCOVER_CATEGORIES, NEARBY_PROVIDERS } from "@/lib/agent";
...
/** Geography-first directory of nearby, agent-reachable providers. */
export function Discover({ navigate }: DiscoverProps) {
  ...
  const results = useMemo(() => {
    ...
    return NEARBY_PROVIDERS.filter(p => {
  ...
  return (
    <DeviceFrame>
      ...
      <main className="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto px-4 pb-24 pt-3">
        <p className="text-[15px] text-on-surface-variant">Taiz understands your city — search or browse nearby.</p>
          ...
          {results.length > 0 ? (
            results.map(provider => (
              <ProviderCard
                key={provider.id}
                provider={provider}
                onClick={() => showToast(`${provider.name} — profile coming soon`)}
              />
            ))
          ) : (
            <EmptyState icon={Search} title="No matching providers nearby" description="Try a different category or search term." />
          )}
        </div>
      </main>
